...
export const StatusBar: React.FC<StatusBarProps> = ({ state, eggValue, productionEstimate }) => {
  return (
    <header className="status-bar rounded-2xl px-4 py-3 sm:px-6 flex flex-wrap gap-3 items-center justify-between">
      <div>
        ...
        <p className="text-xs text-amber-800/70">Small flock strategy</p>
      </div>
      ...
      <div className="flex flex-wrap gap-2 sm:gap-3 text-sm">
        ...
        <StatPill label="Actions" value={`${state.actionsRemaining}/4`} />
        <StatPill label="Reputation" value={`${state.reputation}%`} />
      </div>

      <div className="w-full sm:w-auto flex flex-wrap gap-2 text-xs text-amber-900/80">
        <Meter label="Coop clean" value={state.coopCleanliness} color="#7A9E5E" />
        <Meter label="Water" value={state.waterLevel} color="#5B9BD5" />
        <Meter label="Feed" value={Math.min(100, (state.inventory.feedLbs / 80) * 100)} color="#C9A227" />
        <span className="px-2 py-1 rounded-lg bg-amber-100/80 border border-amber-200">
          Est. {productionEstimate.toFixed(1)} eggs/wk · Cartons worth ${eggValue.toFixed(2)}
        </span>
      </div>
    </header>
  );
...
const StatPill: React.FC<{ label: string; value: string; accent?: boolean }> = ({
  ...
  <div
    className={`px-3 py-1.5 rounded-xl border ${
      accent ? 'bg-amber-200/90 border-amber-300' : 'bg-white/70 border-amber-200/80'
    }`}
  >
...
const Meter: React.FC<{ label: string; value: number; color: string }> = ({ label, value, color }) => (
  <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-white/60 border border-amber-200/60 min-w-[110px]">
    <span className="w-14 text-amber-800/70">{label}</span>
